...
def get_data(args):
    ...
    def convert(srcfile, targetfile, labelfile, batchsize, seqlength, outfile, num_sents, max_sent_l=0, shuffle=0):
        ...
        labels = np.zeros((num_sents,), dtype =float)
        ...
        f["label"] = np.array(labels, dtype=float)
        f["label_size"] = np.array([6])
    ...
    max_sent_l = convert(args.srctestfile, args.targettestfile, args.labeltestfile, args.batchsize, args.seqlength, args.outputfile + "-test.hdf5", num_sents_test, max_sent_l, args.shuffle)    
...
def main(arguments):
    ...
    parser.add_argument('--vocabsize', help="Size of source vocabulary, constructed by taking the top X most frequent words.  Rest are replaced with special UNK tokens.", type=int, default=50000)
    ...
    parser.add_argument('--seqlength', help="Maximum sequence length. Sequences longer than this are dropped.", type=int, default=100)
