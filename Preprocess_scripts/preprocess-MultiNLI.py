...
def get_data(args):
    ...
    def make_vocab(srcfile, targetfile, labelfile, seqlength):
        ...
        for _, (src_orig, targ_orig, label_orig) in enumerate(itertools.izip(open(srcfile,'r'), open(targetfile,'r'), open(labelfile, 'r'))):
            ...
            #todo : add pairID if test (add testfile as argument)
    ...
    def convert(srcfile, targetfile, labelfile, batchsize, seqlength, outfile, num_sents, max_sent_l=0, shuffle=0, pairIDfile=None):
        ...
        if pairIDfile is not None:
            pairIDs = np.zeros((num_sents,), dtype =int)
            pairIDlines = open(pairIDfile,'r').read().split("\n")
        
        dropped = 0
        sent_id = 0
        
        for index, (src_orig, targ_orig, label_orig) in enumerate(itertools.izip(open(srcfile,'r'), open(targetfile,'r') ,open(labelfile,'r'))):
            src_orig = word_indexer.clean(src_orig.strip())
            targ_orig = word_indexer.clean(targ_orig.strip())
            ...
            if pairIDfile is not None:
                pairID = pairIDlines[index].strip()
        ...
        if pairIDfile is not None:
            pairIDs = pairIDs[sorted_lengths]
        ...
        if pairIDfile is not None:
            f["pairID"] = np.array(pairIDs, dtype=int)
    ...
    num_sents_train = make_vocab(args.srcfile, args.targetfile, args.labelfile, args.seqlength)
    print("Number of sentences in training: {}".format(num_sents_train))
    
    num_sents_valid = make_vocab(args.srcvalfile, args.targetvalfile, args.labelvalfile, args.seqlength)
    print("Number of sentences in valid: {}".format(num_sents_valid))
    
    num_sents_test = make_vocab(args.srctestfile, args.targettestfile, args.labeltestfile, args.seqlength)
    ...
    print("Source vocab size: Original = {}, Pruned = {}".format(len(word_indexer.vocab), len(word_indexer.d)))
    print("Target vocab size: Original = {}, Pruned = {}".format(len(word_indexer.vocab), len(word_indexer.d)))

    max_sent_l = 0
    max_sent_l = convert(args.srcvalfile, args.targetvalfile, args.labelvalfile, args.batchsize, args.seqlength, args.outputfile + "-val.hdf5", num_sents_valid, max_sent_l, args.shuffle)
    max_sent_l = convert(args.srcfile, args.targetfile, args.labelfile, args.batchsize, args.seqlength, args.outputfile + "-train.hdf5", num_sents_train, max_sent_l, args.shuffle)
    max_sent_l = convert(args.srctestfile, args.targettestfile, args.labeltestfile, args.batchsize, args.seqlength, args.outputfile + "-test.hdf5", num_sents_test, max_sent_l, args.shuffle, args.pairIDtestfile)    
...
def main(arguments):
    ...
    parser.add_argument('--vocabsize', help="Size of source vocabulary, constructed by taking the top X most frequent words.  Rest are replaced with special UNK tokens.", type=int, default=80000)
    parser.add_argument('--srcfile', help="Path to sent1 training data.", default = "data/entail/src-train.txt")
    parser.add_argument('--targetfile', help="Path to sent2 training data.", default = "data/entail/targ-train.txt")
    parser.add_argument('--labelfile', help="Path to label data, where each line represents a single label for the sentence pair.", default = "data/entail/label-train.txt")    
    parser.add_argument('--srcvalfile', help="Path to sent1 validation data.", default = "data/entail/src-dev.txt")
    parser.add_argument('--targetvalfile', help="Path to sent2 validation data.", default = "data/entail/targ-dev.txt")
    parser.add_argument('--labelvalfile', help="Path to label validation data.", default = "data/entail/label-dev.txt")
    parser.add_argument('--srctestfile', help="Path to sent1 test data.", default = "data/entail/src-test.txt")
    parser.add_argument('--targettestfile', help="Path to sent2 test data.", default = "data/entail/targ-test.txt")
    parser.add_argument('--labeltestfile', help="Path to label test data.", default = "data/entail/label-test.txt")
    parser.add_argument('--pairIDtestfile', help="Path to pairID test values.", default = "data/entail/pairID-test.txt")
    
    parser.add_argument('--batchsize', help="Size of each minibatch.", type=int, default=32)
    parser.add_argument('--seqlength', help="Maximum sequence length. Sequences longer than this are dropped.", type=int, default=1000)
    parser.add_argument('--outputfile', help="Prefix of the output file names. ", type=str, default = "data/entail")
    parser.add_argument('--vocabfile', help="If working with a preset vocab, then including this will ignore vocabsize and use the vocab provided here.", type = str, default='')
